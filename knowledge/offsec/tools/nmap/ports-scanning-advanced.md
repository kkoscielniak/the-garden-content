...
The _null scan_ does not set any flag (all six flag bits in [[knowledge/offsec/tools/nmap/tcp-header]] are set to zero).
...
The result is quite similar to the result we obtained earlier using a [[knowledge/offsec/tools/nmap/ports-scanning-advanced#Null Scan (-sN)|Null Scan]].
...
The obtained results are pretty similar to that of the [[knowledge/offsec/tools/nmap/ports-scanning-advanced#Null Scan (-sN)|Null Scan]] and the [[knowledge/offsec/tools/nmap/ports-scanning-advanced#FIN Scan (-sF)|FIN Scan]].
...
> [!tip] Null, FIN and Xmas scan types can be efficient is when scanning a target behind a stateless (non-stateful) [[knowledge/offsec/hardening/firewall]].
>
...
However, this kind of scan would be helpful if there is a [[knowledge/offsec/hardening/firewall]] in front of the target.
...
The _TCP Window Scan_ is almost the same as [[knowledge/offsec/tools/nmap/ports-scanning-advanced#TCP ACK Scan (-sA)|ACK Scan]], however, it examines the TCP Window field of the RST packets returned.
