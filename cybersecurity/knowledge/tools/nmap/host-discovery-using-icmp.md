...
Remember that an [[cybersecurity/knowledge/tools/nmap/host-discovery-using-arp|ARP]] query will precede the [[cybersecurity/knowledge/glossary/ICMP]] request if we target is on the same [[networks/subnetworks|subnet]].
...
We will scan from a system that belongs to a different subnet. The results are similar to what we've seen in [[cybersecurity/knowledge/tools/nmap/host-discovery-using-arp]] but without the MAC addresses.
